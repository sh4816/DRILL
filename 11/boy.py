...
LSHIFT_DOWN, RSHIFT_DOWN, LSHIFT_UP, RSHIFT_UP, DASH_TIMER_END = range(10)
...
class DashState:
    ...
    def do(boy):
        boy.frame = (boy.frame + 1) % 8
        boy.timer -= 1
        # 1초가 지나면 대시키를 누르고 있어도 Run상태로 바뀜
        if boy.timer == 0:
            boy.add_event(DASH_TIMER_END)
        boy.x += boy.velocity*3 # Run보다 3배 빠르게 이동
        boy.x = clamp(25, boy.x, 800 - 25)
